...
\end{figure}\subsubsection*{Alternatívne toky}
...
\begin{enumerate}
    ...
    Zákazník si prezrie digitálne menu, vyberie hlavné jedlo a drink a potvrdí objednávku. \\
    \textit{Dátová vrstva:} Prehľadáva sa tabuľka \texttt{PolozkaMenu}, pričom sa vrátia len záznamy kde \texttt{je\_dostupna = TRUE} s atribútmi \texttt{nazov}, \texttt{popis}, \texttt{aktualna\_cena} a \texttt{kategoria}. Po potvrdení sa vytvorí nový záznam v tabuľke \texttt{Objednavka} s atribútmi \texttt{stav} nastaveným na \texttt{nova} a \texttt{cas\_vytvorenia}. Následne sa vytvorí prepojený záznam v tabuľke \texttt{DineInObjednavka} obsahujúci \texttt{cislo\_stola} a \texttt{id\_casnika}. Pre každú vybranú položku sa vytvorí samostatný záznam v tabuľke \texttt{ObjednavkaPolozka} s atribútmi \texttt{id\_objednavky}, \texttt{id\_polozky}, \texttt{mnozstvo}, \texttt{stav} nastavený na \texttt{nova} a \texttt{cena\_v\_case\_objednavky} skopírovaná z aktuálnej ceny položky menu.

    \item \textbf{Zákazník dostane objednávku} \\
    Zákazník sleduje stav objednávky, čašník postupne prináša hotové položky na stôl. \\
    \textit{Dátová vrstva:} Prehľadáva sa tabuľka \texttt{Objednavka} podľa \texttt{id} objednávky a vracia sa aktuálna hodnota atribútu \texttt{stav}. Pre každú doručenú položku čašník aktualizuje atribút \texttt{stav} v tabuľke \texttt{ObjednavkaPolozka} na hodnotu \texttt{dorucena}.

    \item \textbf{Zákazník zaplatí} \\
    Zákazník požiada o účet, zvolí spôsob platby a zaplatí. \\
    \textit{Dátová vrstva:} Systém zobrazí súhrn objednávky - prehľadáva sa tabuľka \texttt{ObjednavkaPolozka} prepojená s tabuľkou \texttt{PolozkaMenu} cez \texttt{id\_polozky}. Pre každú položku sa vrátia atribúty \texttt{nazov}, \texttt{mnozstvo}, \texttt{cena\_v\_case\_objednavky} a vypočítaný medzisúčet. Celková suma sa získa agregáciou cez všetky záznamy \texttt{ObjednavkaPolozka} patriace k danej objednávke. Následne sa vytvorí nový záznam v tabuľke \texttt{Faktura} s atribútmi \texttt{id\_objednavky}, \texttt{je\_zaplatena} nastaveným na \texttt{TRUE} a \texttt{sposob\_platby}. V tabuľke \texttt{Objednavka} sa aktualizuje \texttt{stav} na \texttt{zaplatena} a v tabuľke \texttt{Stol} sa \texttt{stav} príslušného stola aktualizuje na \texttt{volny}.
    ...
    \textit{Dátová vrstva:} Systém overí v tabuľke \texttt{Recenzia}, že pre dané \texttt{id\_objednavky} ešte žiadny záznam neexistuje. Ak podmienka platí, vytvorí sa nový záznam s atribútmi \texttt{id\_objednavky}, \texttt{hodnotenie}, \texttt{komentar} a \texttt{cas}.
...
\begin{figure}[H]
    ...
    \caption{Scenár č.2 - Časť 2: Príprava a doručenie jedla:}
...
\begin{figure}[H]
    ...
    \caption{Scenár č.2 - Časť 3: Účet a platba:}
