...
Dopyt aj pohľad \texttt{v\_dostupne\_polozky} používajú podmienku
\texttt{je\_dostupna = TRUE}. Pri overení konkrétnej položky objednávky je však v predikáte aj
\texttt{PolozkaMenu.id}, takže databáza môže namiesto tohto indexu použiť primárny kľúč nad
\texttt{id} a dostupnosť skontrolovať až filtrom. Index je preto najvhodnejší pre časté filtrovanie
dostupného menu; pri malej tabuľke menu môže optimalizátor zvoliť aj sekvenčný sken.
...
podporuje následný join na \texttt{Objednavka.id}. Na aktuálnom seede je však hodnota
\texttt{je\_zaplatena = TRUE} veľmi málo selektívna, pretože zaplatených je 3602 z 3621 faktúr.
Reálny plán preto používa \texttt{Seq Scan on faktura}. Index ostáva opodstatnený pre väčšiu alebo
vyváženejšiu produkčnú databázu, kde filter zaplatených faktúr môže znížiť počet čítaných riadkov.
