...
Proces overuje, že pre daný stôl, zákazníka, dátum a čas existuje potvrdená rezervácia a že všetky
objednávané položky menu existujú, sú dostupné a majú kladné množstvo. Ak kontroly prejdú, v jednom
SQL príkaze sa aktivuje rezervácia a vložia sa záznamy do \texttt{Objednavka},
\texttt{DineInObjednavka} a \texttt{ObjednavkaPolozka}. Referenčnú integritu zákazníka a čašníka
následne chránia cudzie kľúče fyzického modelu.
...
Implementácia je v súbore \texttt{queries.sql}. Proces nepoužíva dočasné tabuľky, PL/pgSQL blok
\texttt{DO}, vlastné uložené funkcie, procedúry, triggery ani kurzory. Celý zápis je jeden SQL
statement. CTE sa používajú iba tam, kde sú potrebné na spoločné vstupy alebo na reťazenie
\texttt{UPDATE}/\texttt{INSERT ... RETURNING}; kontrola položiek menu je riešená cez vnorené
subqueries.
...
activated_reservation AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM input_items
        WHERE mnozstvo <= 0 OR polozka_nie_je_dostupna
    )
    RETURNING id
),
inserted_order AS (
    INSERT INTO Objednavka (cas_vytvorenia)
    SELECT datum + cas_prichodu
    FROM params
...
Objednávka sa vloží iba vtedy, keď CTE \texttt{activated\_reservation} vráti riadok. Toto CTE vráti
riadok len pri nájdenej potvrdenej rezervácii a zároveň pri absencii neplatnej alebo nedostupnej
položky. Nadväzujúce \texttt{INSERT} CTE sú preto prázdne, ak validácia neprejde. Keďže všetky zápisy
sú súčasťou jedného SQL statementu, nevznikne situácia, že by sa vytvorila iba časť objednávky.
...
\begin{itemize}
    \item \texttt{idx\_rezervacia\_stol\_datum} podporuje hľadanie potvrdenej rezervácie podľa stola, dátumu a stavu.
    \item \texttt{idx\_polozka\_dostupna} podporuje filtrovanie dostupného menu; pri kontrole konkrétnej položky podľa ID môže databáza použiť aj primárny kľúč \texttt{PolozkaMenu.id}.
\end{itemize}
...
FROM (
    ...
    LEFT JOIN (
        ...
        JOIN DineInObjednavka d ON d.id = o.id
        WHERE o.cas_vytvorenia >= '2026-01-01'
          AND o.cas_vytvorenia <  '2027-01-01'
...
\begin{itemize}
    ...
    \item \texttt{idx\_faktura\_zaplatena} podporuje výber zaplatených faktúr; na aktuálnom seede však plánovač volí sekvenčný sken, keďže takmer všetky faktúry sú zaplatené.
    \item \texttt{idx\_objednavkapolozka\_objednavka} podporuje agregáciu položiek podľa objednávky.
    \item \texttt{idx\_dinein\_casnik} podporuje filtrovanie a agregáciu objednávok podľa čašníka.
\end{itemize}

Kontrola cez \texttt{EXPLAIN (ANALYZE, BUFFERS)} nad seed dátami potvrdila použitie indexu nad
dátumom vytvorenia objednávky pri filtrovaní roka 2026 a indexu nad cudzím kľúčom položiek pri
pripájaní položiek objednávky. Pri tabuľke \texttt{Faktura} plánovač zvolil sekvenčný sken, pretože
filter \texttt{je\_zaplatena = TRUE} v seede vyberá takmer všetky riadky.

\clearpage
\section{Výkonnosť a indexy}

PostgreSQL automaticky vytvára indexy pre \texttt{PRIMARY KEY} a \texttt{UNIQUE} obmedzenia.
Tieto indexy sa do požadovaného minima nepočítajú. V schéme preto vytvárame šesť vlastných
...
Ide o partial index, takže obsahuje iba riadky, kde

\texttt{je\_dostupna = TRUE}. Keďže proces pracuje iba s dostupnými položkami menu,
index je menší než plný index nad všetkými riadkami tabuľky.
Index podporuje filtrovanie dostupných položiek menu a zároveň umožňuje efektívne
vyhľadanie konkrétnej dostupnej položky podľa \texttt{id}. Pri dotazoch typu:
\begin{lstlisting}[language=SQL]
SELECT *
FROM PolozkaMenu
WHERE je_dostupna = TRUE
...
môže PostgreSQL použiť priamo tento partial index namiesto sekvenčného skenu
celej tabuľky. Pri malej tabuľke menu však optimalizátor môže stále zvoliť
sekvenčný sken, pretože jeho cena môže byť nižšia než použitie indexu.
...
Index podporuje najmä proces 2 pri agregácii tržby objednávky, kde sa položky pripájajú cez
\texttt{ObjednavkaPolozka.id\_objednavky}. V aktuálnej verzii procesu 1 sa detail práve vytvorenej
objednávky skladá z CTE \texttt{inserted\_items}, takže tento index nie je potrebný na finálny výstup
procesu 1. Ostáva však dôležitý pri bežnom neskoršom načítaní detailu objednávky podľa
\texttt{id\_objednavky} a pri analytickom reporte. PostgreSQL nevytvára index automaticky na stĺpci
cudzieho kľúča, preto je tento index vlastný.
...
\subsection*{\texttt{idx\_faktura\_nezaplatena}}

\begin{lstlisting}[language=SQL]
CREATE INDEX idx_faktura_nezaplatena
    ON Faktura (id_objednavky)
    WHERE je_zaplatena = FALSE;
\end{lstlisting}

Index podporuje proces 2 pri vyhľadávaní nezaplatených faktúr.
Keďže atribút \texttt{je\_zaplatena} obsahuje iba dve hodnoty, partial index je efektívnejší
ako klasický B-tree index nad boolean stĺpcom. Index obsahuje iba riadky so stavom
\texttt{FALSE}, čím znižuje veľkosť indexu aj počet čítaných stránok.

\subsection*{\texttt{idx\_dinein\_casnik}}

\begin{lstlisting}[language=SQL]
CREATE INDEX idx_dinein_casnik
    ON DineInObjednavka (id_casnika);
\end{lstlisting}

Index podporuje proces 2 pri filtrovaní a agregácii objednávok podľa čašníka.
Report zoskupuje dine-in objednávky podľa \texttt{id\_casnika}, počíta tržby,
počet objednávok a priemerné hodnotenia jednotlivých čašníkov. Bez indexu musí
databáza prechádzať celú tabuľku \texttt{DineInObjednavka} sekvenčným skenom pri
vyhľadávaní objednávok konkrétneho čašníka.
...
\begin{lstlisting}
Limit  (actual time=0.237..0.237 rows=1 loops=1)
  Buffers: shared hit=13
  ->  Seq Scan on rezervacia r
        Rows Removed by Filter: 1376
        Buffers: shared hit=13
...
1 500 rezervácií a v tomto teste zahodila 1 376 riadkov filtrom, kým našla vyhovujúcu rezerváciu.
...
\begin{lstlisting}
Limit  (actual time=0.015..0.015 rows=1 loops=1)
...
vykonania z približne \texttt{0.254 ms} na \texttt{0.032 ms}. Zvyšné podmienky na zákazníka a časový
interval sa aplikujú už iba na kandidátov nájdených cez index.
